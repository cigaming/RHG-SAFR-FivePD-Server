...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			if GetEntityHealth(GetPlayerPed(-1)) <= 100 then --You may need to edit this death check for your server
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Press [lctrl] to release, [q] to kill")
			if IsDisabledControlJustPressed(0,60) then --release
				print("release this mofo")			
				holdingHostage = false
				holdingHostageInProgress = false 
				--ClearPedSecondaryTask(GetPlayerPed(-1))
				...
				Wait(100)
				releaseHostage()
			elseif IsDisabledControlJustPressed(0,52) then --kill 
		...
		if beingHeldHostage then 
			...
			DisableControlAction(0,73,true) -- disable X to stop ppl canceling animation
		end
		Wait(0)
	end
end)
