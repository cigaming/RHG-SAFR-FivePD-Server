...
AddEventHandler("knb:mech", function()
    ...
    if DoesEntityExist(targetVeh) then
		...
		exports['mythic_notify']:DoHudText('error', 'Cannot be used because there are enough EMS online.')
		else
...
function GoToTarget(x, y, z, vehicle, driver, vehhash, player)
    ...
    ShowAdvancedNotification(companyIcon, companyName, "EMS has been notified"," A ambulance has been sent to your area. ~y~" .. companyName)
    enroute = true
    while enroute do
        Citizen.Wait(500)
...
function RepairVehicle(player, vehicle, driver)
    ...
    Citizen.Wait(500)
	ShowAdvancedNotification(mechPedPick.icon, mechPedPick.name, "Pillbox Hill EMS" , mechPedPick.lines[math.random(#mechPedPick.lines)])
