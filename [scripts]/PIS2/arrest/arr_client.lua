...
AddEventHandler('pis:arr:pt', function()
    ...
    RequestModel(driver)
		...
		if DoesEntityExist(targetPed) then
			...
			ShowNotification('~o~Officer: ~w~Dispatch, requesting Prisoner Trasport to my 20.')
			...
			ShowNotification('~o~Dispatch: ~w~A prisoner transport van has been dispatched to your location. ~w~\nETA: ' .. eta)
			enroute = true
			while (enroute) do
			Citizen.Wait(300)
					...
					if GetEntitySpeed(vehicle) < 5 then
						...
						if isopen ~= 0 then
							...
							if IsPedInVehicle(targetPed, vehicle, false) then
								...
								ShowNotification("~o~Van Driver:~w~ I've got it from here.")
								enroute = false
...
AddEventHandler('pis:arr:handcuff', function()	-- on /handcuff
	...
	if DoesEntityExist(cuffingPed) then -- if the ped exits in front of the player.
		SetEntityAsMissionEntity(cuffingPed)
	else
	...
	if IsEntityPlayingAnim(cuffingPed, "random@arrests@busted", "idle_a", 3) then -- if the person is doing "kneeling, hands behind head" animation, put him under arrest.
		...
		TaskPlayAnim(cuffingPed, "mp_arresting", "idle", 8.0, -8, -1, 1, 15, 0, 0, 0)
...
RegisterCommand('seatped', function(source, args, rawCommand)

player = GetPlayerPed(-1)

carr = GetVehiclePedIsIn(player)

cuffed = false



TaskWarpPedIntoVehicle(cuffingPed, carr, 1)
TriggerEvent('secure')
--Wait(3000)
...
RegisterCommand('bookped', function(source, args, rawCommand)

DeleteEntity(cuffingPed)
...
AddEventHandler('pis:arr:book', function()
    local player = GetPlayerPed(-1) --Player
        ...
        local playerPos = GetEntityCoords( player )
        local inFrontOfPlayer = GetOffsetFromEntityInWorldCoords( player, 0.0, 7.0, 0.0 )
        local pedPosition = GetEntityCoords(cuffingPed)
        local distanceBetween =  GetDistanceBetweenCoords(playerPos, pedPosition)
        if DoesEntityExist(cuffingPed) and distanceBetween < 7 then
            DeletePed(cuffingPed)
        else
        
        local bookingPed = GetPedInDirection( playerPos, inFrontOfPlayer )
        
        DeletePed(bookingPed)
        TriggerClientEvent('po:release')
        end
    end)
end)
