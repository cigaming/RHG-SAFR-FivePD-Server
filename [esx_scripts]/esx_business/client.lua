    ...
    ESX.TriggerServerCallback("esx_business:getStock", function(stock)
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_boss',
        {
            ...
            elements = {
                ...
                {label = 'Sell business - <span style="color:red;">$'..tostring(math.floor(business["price"]*0.8))..'</span>', value = 'sellbusiness'},
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_create',
    {
        ...
        elseif data.current.action == 'create' then
            ...
            if not error then
                ...
                ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_create_actionpos', {title="Please select the boss action location",align = 'right',elements={{label='<span style="color:red;">Your current position will be used</span>',action=''},{label="Set position",action="setpos"},{label="Cancel",action="cancel"}}},function(data1,menu1) if data1.current.action=="setpos" then 
                    local business = {
...
AddEventHandler("esx_business:businessList",function()
    if #businessData<1 then Citizen.Trace("No business info on client, if there is some on server, you should try running the `/business reload` command to sync server info") else
        ...
        for k,v in ipairs(businessData) do
            ...
            Citizen.Trace("|-> Owner: "..v["owner_name"].." ("..v["owner"]..")")
...
Citizen.CreateThread(function()
    while true do
        ...
        Citizen.Wait(0)
        local inbuypoint = function() local ip = false; for _,v in ipairs(businessData) do ip = ip or GetDistanceBetweenCoords(vector3(v.position.buy.x,v.position.buy.y,v.position.buy.z),GetEntityCoords(ped),false)<2.0 end; return ip end
        for id,business in ipairs(businessData) do
            ...
            if GetDistanceBetweenCoords(vector3(x,y,z), GetEntityCoords(ped), false)<50 then
                DrawText3D(x,y,z+0.23, "~y~"..business["name"], 1.5)
                DrawText3D(x,y,z+0.0, tostring(business["address"]).."; "..tostring(business["description"]), 0.7)
                DrawText3D(x,y,z-0.1, "~b~Owner: ~r~"..business["owner_name"].." ~b~Price: ~g~$"..ESX.Math.GroupDigits(business["price"]),1)
                DrawText3D(x,y,z-0.21, "~b~Earnings: ~g~$"..ESX.Math.GroupDigits(business["earnings"]).."/h",1)
                if business["owner"]==nil then
                    DrawMarker(29, x, y, z-0.7, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.2001, 0, 255, 0, 200, 0, 0, 0, true)
                    if GetDistanceBetweenCoords(vector3(x,y,z),GetEntityCoords(ped),false)<2.0 and not buy then
                        ESX.ShowHelpNotification('Hit ~INPUT_CONTEXT~ to buy business')
                        if IsControlJustPressed(0, 51) then OpenBuyBusinessMenu(business) end
                    elseif GetDistanceBetweenCoords(vector3(x,y,z),GetEntityCoords(ped),false)>2.0 and not inbuypoint() and buy then
                ...
                elseif business["owner"]==PlayerData.identifier then
                    ...
                    if GetDistanceBetweenCoords(vector3(xx,yy,zz),GetEntityCoords(ped),false)<1.0 and not boss then
                        ...
                        if IsControlJustPressed(0, 51) then OpenBusinessMenu(business) end
                    elseif GetDistanceBetweenCoords(vector3(xx,yy,zz),GetEntityCoords(ped),false)>1.0 and boss then
