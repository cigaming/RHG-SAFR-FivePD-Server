...
RegisterServerEvent('esx_vangelico_robbery2:toofar')
AddEventHandler('esx_vangelico_robbery2:toofar', function(robb)
	local source = source
	...
	for i=1, #xPlayers, 1 do
 		...
 		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('esx_vangelico_robbery2:killblip', xPlayers[i])
		end
	end
	if(robbers[source])then
		TriggerClientEvent('esx_vangelico_robbery2:toofarlocal', source)
		robbers[source] = nil
		TriggerClientEvent('esx:showNotification', source, _U('robbery_has_cancelled') .. Stores[robb].nameofstore)
	end
end)

RegisterServerEvent('esx_vangelico_robbery2:endrob')
AddEventHandler('esx_vangelico_robbery2:endrob', function(robb)
	local source = source
	local xPlayers = ESX.GetPlayers()
	rob = false
	for i=1, #xPlayers, 1 do
	...
	if(robbers[source])then
		TriggerClientEvent('esx_vangelico_robbery2:robberycomplete', source)
		robbers[source] = nil
		TriggerClientEvent('esx:showNotification', source, _U('robbery_has_ended') .. Stores[robb].nameofstore)
	end
end)

RegisterServerEvent('esx_vangelico_robbery2:rob')
AddEventHandler('esx_vangelico_robbery2:rob', function(robb)
	...
	if Stores[robb] then
		...
		if (os.time() - store.lastrobbed) < 600 and store.lastrobbed ~= 0 then
            ...
            TriggerClientEvent('esx_vangelico_robbery2:togliblip', source)
			...
			if(cops >= Config.RequiredCopsRob)then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
							...
							TriggerClientEvent('esx_vangelico_robbery2:setblip', xPlayers[i], Stores[robb].position)
					end
				end
			    ...
			    TriggerClientEvent('esx_vangelico_robbery2:currentlyrobbing', source, robb)
                CancelEvent()
...
RegisterServerEvent('esx_vangelico_robbery2:gioielli1')
AddEventHandler('esx_vangelico_robbery2:gioielli1', function()
...
RegisterServerEvent('lester2:vendita')
AddEventHandler('lester2:vendita', function()
...
RegisterServerEvent('lester2:nvendita')
AddEventHandler('lester2:nvendita', function()
	local _source = source
