        ...
        if PlayerData.job ~= nil and PlayerData.job.name ~= "police" then
            ...
            for detector, v in pairs(Config.Detectors) do
                ...
                if distanceCheck <= 10.0 then
                    ...
                    ESX.Game.Utils.DrawText3D({ x = v.x, y = v.y, z = v.z }, "Detector", 0.5)

                    if distanceCheck <= 1.5 then
                        
                        if IsPedOnFoot(ped) then
                            
                            ESX.ShowNotification("Police have been alerted")

                            TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 25, "detector", 1.0)

                            Citizen.Wait(100)
                            
                            TriggerServerEvent('esx_phone:send', 'police', "Convicts have breached DOC perimeter", true, {}, true)
                            break
                        end
                    end
                end
