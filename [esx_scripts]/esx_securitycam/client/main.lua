			...
			if PlayerData.job ~= nil and PlayerData.job.name == "taxi" then
				if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
					...
					if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 1.5 then
						DrawText3D(v.Pos.x, v.Pos.y, v.Pos.z+1.5, "~w~[~g~E~w~] Use Cameras.", 0.80)
					end
				end
			end
		end
		...
		if IsControlJustReleased(0, Keys['E']) and CurrentAction == 'cameras' then
			...
			if not menuopen then
				...
				local elements = {
					...
					{label = _U('field_menu_selection'), value = 'fields'},
				}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
				{
					...
					elseif data.current.value == 'fields' then
						...
						if policeHacked then
							...
							fieldcamera = true
							blockbuttons = true
							local firstCamx = Config.Locations[1].fieldCameras[1].x
							local firstCamy = Config.Locations[1].fieldCameras[1].y
							local firstCamz = Config.Locations[1].fieldCameras[1].z
							local firstCamr = Config.Locations[1].fieldCameras[1].r
							...
							SendNUIMessage({
								type  = "enablecam",
								label = Config.Locations[1].fieldCameras[1].label,
								box   = Config.Locations[1].fieldCameras.label
							})
		...
		if createdCamera ~= 0 then
			...
			if IsControlJustPressed(0, Keys["LEFT"]) then
				...
				elseif policecamera then
					...
					ChangeSecurityCamera(newCamx, newCamy, newCamz, newCamr)
					currentCameraIndexIndex = newCamIndex
				elseif fieldcamera then
					local newCamIndex

					if currentCameraIndexIndex == 1 then
						newCamIndex = #Config.Locations[currentCameraIndex].fieldCameras
					else
						newCamIndex = currentCameraIndexIndex - 1
					end

					local newCamx = Config.Locations[currentCameraIndex].fieldCameras[newCamIndex].x
					local newCamy = Config.Locations[currentCameraIndex].fieldCameras[newCamIndex].y
					local newCamz = Config.Locations[currentCameraIndex].fieldCameras[newCamIndex].z
					local newCamr = Config.Locations[currentCameraIndex].fieldCameras[newCamIndex].r

					SetFocusArea(newCamx, newCamy, newCamz, newCamx, newCamy, newCamz)
					SendNUIMessage({
						type = "updatecam",
						label = Config.Locations[currentCameraIndex].fieldCameras[newCamIndex].label
					})
			...
			if IsControlJustPressed(0, Keys["RIGHT"]) then
				...
				elseif fieldcamera then
					...
					if currentCameraIndexIndex == #Config.Locations[currentCameraIndex].fieldCameras then
						newCamIndex = 1
					else
						newCamIndex = currentCameraIndexIndex + 1
					end
			...
			elseif Config.Locations[currentCameraIndex].fieldCameras[currentCameraIndexIndex].canRotate then
				local getCameraRot = GetCamRot(createdCamera, 2)
