	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
				...
				SetVehicleNumberPlateText(vehicle, "PACKER"..platenum)             
				plaquevehicule = "PACKER"..platenum
				...
				if data.current.value == 'phantom3' then
					ESX.Game.SpawnVehicle("trailers2", Config.PickUps.TrailerSpawnPoint.Pos, Config.PickUps.TrailerSpawnPoint.Heading, function(trailer)
						

						SetWaypointOff()
						SetNewWaypoint(Config.PickUps.TrailerSpawnPoint.Pos.x, Config.PickUps.TrailerSpawnPoint.Pos.y)
						ESX.ShowNotification("~g~GPS Marked!~w~Pick up the ~b~Cargo")

					end)
				end	
				if data.current.value == 'phantom' then
					ESX.Game.SpawnVehicle("trailers2", Config.PickUps.TrailerSpawnPoint2.Pos, Config.PickUps.TrailerSpawnPoint2.Heading, function(trailer)

						SetWaypointOff()
						SetNewWaypoint(Config.PickUps.TrailerSpawnPoint2.Pos.x, Config.PickUps.TrailerSpawnPoint2.Pos.y)
						ESX.ShowNotification("~g~GPS Marked!~w~Pick up the ~b~Cargo")

					end)
				end	
				if data.current.value == 'hauler' then
					ESX.Game.SpawnVehicle("trailers2", Config.PickUps.TrailerSpawnPoint3.Pos, Config.PickUps.TrailerSpawnPoint3.Heading, function(trailer)
						
						SetWaypointOff()
						SetNewWaypoint(Config.PickUps.TrailerSpawnPoint3.Pos.x, Config.PickUps.TrailerSpawnPoint3.Pos.y)
						ESX.ShowNotification("~g~GPS Marked!~w~Pick up the ~b~Cargo")

					end)
				end	
				if data.current.value == 'packer' then
					ESX.Game.SpawnVehicle("trailers2", Config.PickUps.TrailerSpawnPoint4.Pos, Config.PickUps.TrailerSpawnPoint4.Heading, function(trailer)
						
						SetWaypointOff()
						SetNewWaypoint(Config.PickUps.TrailerSpawnPoint4.Pos.x, Config.PickUps.TrailerSpawnPoint4.Pos.y)	
						ESX.ShowNotification("~g~GPS Marked!~w~Pick up the ~b~Cargo")
...
Citizen.CreateThread(function()
	while true do
		...
		if IsJobTrucker() and IsATruck() then
			if IsVehicleAttachedToTrailer(GetVehiclePedIsUsing(playerPed)) and not MissionLivraison then
...
function donnerlapaye()
	...
	vievehicule = GetVehicleBodyHealth(vehicle)
...
function MissionLivraisonStopRetourDepot()
	...
	DetachVehicleFromTrailer(GetVehiclePedIsIn(GetPlayerPed(-1), false))
