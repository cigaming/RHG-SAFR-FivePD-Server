		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
				...
				if chance == true then
					...
					ESX.ShowNotification('Lockpicking vehicle')
                ...
                if CurrentAction ~= nil then
                    ...
                    ESX.ShowNotification('Unjamming handbrake')
	...
	while true do
		...
		if DoesEntityExist(GetVehiclePedIsTryingToEnter(PlayerPedId())) then
			...
			local lock = GetVehicleDoorLockStatus(veh)
				...
				if ((lock == 7) or (lock == 2) or (lock == 4) or (lock == 5) or (pedd ~= 0 )) then
					if has_value(Config.job_whitelist, xPlayer.job.name) then
						...
						if not lucky or blacklisted then
							...
							TriggerServerEvent('esx_lockpick:setVehicleDoorsForEveryone', {veh, 0, plate})
						end
					end
				end
			end
